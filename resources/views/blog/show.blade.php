...
<article class="container-custom px-4 sm:px-6 lg:px-8 pb-16">
    <div class="max-w-3xl mx-auto">
        ...
        <div class="rounded-2xl overflow-hidden mb-8">
            <x-media-image :model="$blogPost" collection="featured_image" size="large" :eager="true" :alt="$blogPost->title" class="w-full" />
        </div>
        @endif
        ...
        <div class="grid md:grid-cols-3 gap-6">
            ...
            <a href="{{ route('blog.show', $related) }}" class="group">
                <div class="aspect-video bg-surface-100 rounded-lg overflow-hidden mb-3">
                    <x-media-image :model="$related" collection="featured_image" size="medium" :alt="$related->title" class="w-full h-full object-cover group-hover:scale-105 transition-transform" />
                </div>
